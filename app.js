...
// The Bouncer Middleware (Prevents banned IPs from accessing the site)
...
app.get('/api/health', (req, res) => {
    ...
    res.json({
        ...
        loadMem: parseFloat(memPercent.toFixed(3)),
...
app.post('/admin/ban', (req, res) => {
    ...
    io.sockets.sockets.forEach((socket) => {
        ...
        if (socketIp === ip) {
            socket.emit('force-disconnect-banned'); // Tell their client.js to lock the screen
            socket.disconnect(true);                // Sever the actual connection
        }
    });
